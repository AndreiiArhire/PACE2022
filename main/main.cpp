...
long long getFitness(int node) {
    ...
    /*
    if (fitnessType == 3) {
        return min(inDegreeSimple[node].size(), outDegreeSimple[node].size()) + outDegreeDouble[node].size() * 1e11;
    }
    */
    if (fitnessType == 1) {
...
void doLocalSearch() {
    ...
    size = local.size();
    //cout << "--" << size << '\n';
    int toBeErasedCounter = size * 1 / 2;
...
signed main() {
    ...
    return 0;
    */
    for (testNo = 181; testNo <= 181; testNo += 2) {
        //cout << testNo << ' ';
        solveTestcase();
        //cout << getElapsed() << '\n';
    }
